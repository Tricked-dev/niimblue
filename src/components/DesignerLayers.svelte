  ...
  {#each objects as obj (obj)}
    ...
    >
      ...
      <span class="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
        ...
        ><MdIcon icon="keyboard_arrow_up" /></button>
        <button
          class="w-4 h-4 flex items-center justify-center text-zinc-500 hover:text-zinc-200 transition-colors text-[11px]"
          title="Move down"
          onclick={(e) => moveDown(obj, e)}
        ><MdIcon icon="keyboard_arrow_down" /></button>
        <button
          class="w-4 h-4 flex items-center justify-center transition-colors text-[11px] {obj.visible ? 'text-zinc-500 hover:text-zinc-200' : 'text-zinc-600 hover:text-zinc-400'}"

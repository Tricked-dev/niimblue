<script lang="ts">
  ...
  const toolButtons: { type: OjectType; icon: MaterialIcon }[] = [
    { type: "text", icon: "title" },
    { type: "barcode", icon: "view_week" },
    { type: "qrcode", icon: "qr_code_2" },
    { type: "aruco", icon: "grid_on" },
    { type: "rectangle", icon: "crop_square" },
    { type: "circle", icon: "radio_button_unchecked" },
    { type: "line", icon: "remove" },
    { type: "image", icon: "image" },
  ];
</script>

<aside class="flex flex-col items-center w-[42px] bg-zinc-950 border-r border-zinc-800 py-1.5 gap-0.5 shrink-0">
  {#each toolButtons as { type, icon } (type)}
    <button class={btnClass} onclick={() => onObjectPicked(type)}>
      <MdIcon {icon} />
    </button>
  {/each}
  ...
  <button class={btnClass} onclick={onLabelSettingsOpen}>
